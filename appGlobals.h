...
#define INCLUDE_HASIO \
  false  // mqtt.cpp (Send home assistant discovery messages). Needs INCLUDE_MQTT true
...
#define INCLUDE_RTSP \
  false  // rtsp.cpp (RTSP Streaming). Requires additional library: ESP32-RTSPServer
#define INCLUDE_DS18B20 \
  false  // if true, requires INCLUDE_PERIPH and additional libraries: OneWire and DallasTemperature
#define INCLUDE_I2C true  // periphsI2C.cpp (support for I2C peripherals)
#define INCLUDE_ACCELEROMETER \
  (INCLUDE_I2C && INCLUDE_PERIPH)  // accelerometer.cpp (support ADXL345)
...
#define DBG_LVL \
  ESP_LOG_ERROR  // level to use if DBG_ON true: ESP_LOG_ERROR, ESP_LOG_WARN, ESP_LOG_INFO,
...
extern int
    detectNightFrames;  // frames of sequential darkness to avoid spurious day / night switching
...
extern float motionVal;      // motion sensitivity setting - min percentage of changed pixels that
...
const frameStruct frameData[] = {
    ...
    {"QQVGA", 160, 120, 30, 1, 1},   {"128X128", 128, 128, 30, 1, 1},  // PY260
    {"QCIF", 176, 144, 30, 1, 1},    {"HQVGA", 240, 176, 30, 2, 1},
    {"240X240", 240, 240, 30, 2, 1}, {"QVGA", 320, 240, 30, 2, 1},  // PY260
    {"320X320", 320, 320, 30, 2, 1},                                // PY260
    {"CIF", 400, 296, 30, 2, 1},     {"HVGA", 480, 320, 30, 2, 1},
    {"VGA", 640, 480, 20, 3, 1},  // PY260
    {"SVGA", 800, 600, 20, 3, 1},    {"XGA", 1024, 768, 5, 3, 1},
    {"HD", 1280, 720, 5, 3, 1},                                      // PY260
    {"SXGA", 1280, 1024, 5, 3, 1},   {"UXGA", 1600, 1200, 5, 4, 1},  // PY260
    {"FHD", 1920, 1080, 5, 3, 1},                                    // 3MP Sensors only // PY260
    {"P_HD", 720, 1280, 5, 3, 1},                                    //
    {"P_3MP", 864, 1536, 5, 3, 1},                                   //
    {"QXGA", 2048, 1536, 5, 4, 1},   {"QHD", 2560, 1440, 5, 4, 1},   // 5MP Sensors only
